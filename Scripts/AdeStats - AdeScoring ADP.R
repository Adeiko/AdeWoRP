...
ScatMerge <- ggscatter(WORPADP_pred,
  ...
  color = "Pos", fill= "Pos",
  ylim = c(0,max(WORPADP_pred$WinSoRP,na.rm=TRUE)), breaks = c(seq(from = 0, to = max(WORPADP_pred$WinSoRP,na.rm=TRUE), by = 0.5)),
  # xlim = c(1,ADP_Players)
  )+
  ...
  labs(title=paste0("**WoRP** predictions ",dplyr::case_when(Start_rate==FALSE ~ "", TRUE ~ "StartRate "),"by **ADP Posrank**"),
       subtitle=paste0("Stats from Week 1-16 years ",ADP_MinTopYear,"-",ADP_MaxTopYear," and ADP from Redraft"),
       caption = paste0("**Data:** @PFF | **ADP:** @MyFantasyLeague | ",dplyr::case_when(Start_rate==FALSE ~ "", TRUE ~ "**StartRate:** @SleeperHQ | "),"**Plot:** @Adeiko_ff<br>
...
PlotADP <- function(FilPos, TopPlayers, DotColor){
  ...
  WoRPADP_scat = ggscatter(WoRPADP_df,
                           ...
                           ylim = c(-0.5, yrng[2]), breaks = c(seq(from = -0.5, to = yrng[2], by = 0.5)))+
                           # add = "loess", cor.coeff.args = list(method = "pearson", label.x = 3, label.sep = "\n"),
...
ScatGrid <- annotate_figure(
            ggarrange(ScatQB,ScatRB,ScatWR,ScatTE,ncol = 2, nrow = 2,common.legend = TRUE, legend="bottom"),
                      top = richtext_grob(paste0("**WoRP** (Week 1-16) ",dplyr::case_when(Start_rate==FALSE ~ "", TRUE ~ "StartRate "),"predictions by **ADP Posrank** ",ADP_MinTopYear,"-",ADP_MaxTopYear)),
                      bottom = richtext_grob(paste0("**Data:** @PFF | **ADP:** @MyFantasyLeague | ",dplyr::case_when(Start_rate==FALSE ~ "", TRUE ~ "**StartRate:** @SleeperHQ | "),"**Plot:** @Adeiko_ff"),hjust = 1, x = 1))
