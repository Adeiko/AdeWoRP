...
WORPGT_df <- WORP %>%
  ...
  dplyr::mutate(
    aWORP = as.double(format(round(mean(WinSoRP),2), digits=2, nsmall = 2))
  )%>%
...
WORPGT_graph <-WORPGT_df %>%
  ...
  dplyr::select("Player"=headshot_url,"Name"=Player_Name,"Team"=team_logo_espn,"Age"=Physical_Age,WinSoRP,FPSoRP,Pos)%>%
  ...
  gt_theme_538() %>%
  fmt_number(columns = c("Age","WinSoRP","FPSoRP"), decimals = 1) %>%
  # data_color(
  #   columns = c("Age"),
  #   colors = scales::col_quantile(palette = c("#28B8D5","#DC1616"), domain = NULL)
  # )%>%
  tab_source_note(
...
gt::gtsave(
  data = WORPGT_graph,
  filename = paste0("Images/WORP_GT_",WoRP_MinTopYear,"-",WoRP_MaxTopYear,"_",as.numeric(Sys.time()),".png"),
)

WORP_plot <- WORPGT_df %>%
  dplyr::filter(Year>=WoRP_SingleYear)%>%
  dplyr::arrange(desc(aWORP))%>%
  dplyr::slice(1:50)%>%
  dplyr::mutate(Rank=rank(-aWORP,ties.method="min"))%>%
  ggplot() +
  geom_col(
    aes(
      x = aWORP, y = fct_reorder(Player_Name, aWORP),
      fill = Pos,
    ),
    width = 0.7
  ) +
  geom_text(
    aes(x = aWORP, y = fct_reorder(Player_Name, aWORP), label = paste0(Rank," - ",Pos,PosRank)),
    hjust = -0.1, size = 3,
    position = position_stack(vjust = 0),
    inherit.aes = TRUE
  )+
  geom_text(
    aes(x = aWORP, y = fct_reorder(Player_Name, aWORP), label = format(aWORP, nsmall = 2)),
    hjust = -0.5, size = 3.5,
    position = position_dodge(width = 1),
    inherit.aes = TRUE
  )+

  ggimage::geom_image(aes(x = -0.05, y = fct_reorder(Player_Name, aWORP), image = headshot_url),
  size = 0.03, by = "width",
  ) +
